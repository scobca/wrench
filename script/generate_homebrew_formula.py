...
def generate_bottle_block(version: str, repo: str, assets: List[str]) -> str:
    """
    Generate bottle block with correct SHA256 for each platform.
    Now uses the Homebrew-named files (symlinks/duplicates) in the release.
    """
    bottle_lines = []
    base_url = f"https://github.com/{repo}/releases/download/{version}"

    for asset in assets:
        if 'Windows' in asset or 'Source' in asset:
            continue

        bottle_key = get_bottle_key(asset)
        if not bottle_key:
            continue

        bottle_filename = f"wrench-{version}.{bottle_key}.bottle.tar.gz"
        bottle_url = f"{base_url}/{bottle_filename}"

        try:
            sha256 = download_and_calculate_sha256(bottle_url)
            bottle_lines.append(f'    sha256 cellar: :any_skip_relocation, {bottle_key}: "{sha256}"')
            print(f"  -> Added bottle for {bottle_key}")
        except Exception as e:
            print(f"  ⚠️ Error downloading {bottle_filename}: {e}")
            continue
...
def generate_formula(
    ...
    """Generate Homebrew formula with proper bottle block."""

    bottle_block = generate_bottle_block(version, repo, assets)
...
def main():
    ...
    if args.source_sha256:
        ...
        print(f"Using provided source SHA256: {source_sha256}")
    else:
        print(f"Downloading source archive to calculate SHA256...")
